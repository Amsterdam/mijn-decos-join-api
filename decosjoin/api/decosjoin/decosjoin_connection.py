    ...
    def transform(self, zaken, user_identifier):  # noqa: C901
        ...
        for zaak_source in zaken:
            ...
            if self.is_list_match(new_zaak, 'decision', ['buiten behandeling']):
                continue
            if new_zaak['description'] and new_zaak['description'].lower().startswith("*verwijder"):
                continue

            # This url can be used to retrieve matching document attachments for this particular zaak
            new_zaak['documentsUrl'] = f"/api/decosjoin/listdocuments/{encrypt(zaak_source['key'], user_identifier)}"

            if new_zaak['caseType'] == 'Vakantieverhuur afmelding':
